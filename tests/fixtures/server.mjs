...
<script>const status=document.querySelector('[data-testid=status]');document.querySelector('[data-testid=apply]').addEventListener('click',()=>status.textContent='applied');document.querySelector('[data-testid=command]').addEventListener('keydown',event=>{if(event.key==='Enter')status.textContent='entered'});</script></body></html>`;

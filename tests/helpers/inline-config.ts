 ...
 *  defineConfig(...) statement written beside (never inside) the wiped output directory. The
 *  recording block is imported from tests/fixtures/hoolypane.config.ts — its single home — so
 *  generated configs cannot drift from the checked-in one. */
export function inlineConfigSource(baseURL: string, viewports: readonly ViewportSpec[], timeoutMs?: number): string {
  ...
  return `import { defineConfig } from "@hoolypane/runner"; export default defineConfig({ baseURL: "${baseURL}"${timeout}, viewports: [${viewports.map(viewportLiteral).join(", ")}], ${FIXTURE_RECORDING_TAIL} });`;
}

...
function App(): preact.JSX.Element {
  ...
  useEffect(() => window.hoolypaneChrome.subscribe((value) => {
    if (isWorkspace(value)) { dispatch({ type: "state", state: value }); setAddress(value.sharedUrl); }
  }), []);
  useEffect(() => {
    const emit = () => {
    ...
    const request = () => { if (!snapshotPending.current) { snapshotPending.current = true; requestAnimationFrame(emit); } };
  ...
  const navigate = (event: SubmitEvent) => { event.preventDefault(); send({ kind: "navigate", url: address }); };
  return <main className={`chrome chrome-${state.layout}`}>
    <header className="toolbar">
      <form onSubmit={navigate} className="address-form"><label for="address">Address</label><input id="address" value={address} onInput={(event) => setAddress((event.currentTarget as HTMLInputElement).value)} /><button type="submit">Go</button></form>
    ...
    <section className="pane-grid" aria-label="Browser panes">{state.order.map((paneId, index) => {
      ...
      return <article className={`pane-card ${state.focusedPaneId === pane.id ? "focused" : ""}`} key={pane.id}>
        ...
        <footer><label>Name <input value={pane.name} onChange={(event) => send({ kind: "rename", paneId: pane.id, name: (event.currentTarget as HTMLInputElement).value })} /></label><button type="button" onClick={() => send({ kind: "reorder", paneId: pane.id, index: Math.max(0, index - 1) })}>←</button><button type="button" onClick={() => send({ kind: "reorder", paneId: pane.id, index: index + 1 })}>→</button>{pane.failure && <span role="alert">{pane.failure}</span>}{pane.outOfSync && <span role="alert">Out of sync at {pane.outOfSync.actionKind} #{pane.outOfSync.actionId}: {pane.outOfSync.reason}</span>}</footer>
      </article>;
    })}</section>

...
import { errorMessage, gridCellPosition, gridDimensions } from "@hoolypane/contracts";
...
async function render(input: OverviewInput): Promise<Buffer> {
  ...
  const { columns, rows } = gridDimensions(input.tiles.length);
  const overlays: OverlayOptions[] = [];
  for (const [index, tile] of input.tiles.entries()) {
    const { left, top } = gridCellPosition(index, columns, tileWidth, tileHeight);

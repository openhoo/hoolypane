...
for (const manifest of manifests) {
  ...
  for (const section of DEPENDENCY_TYPES) {
    for (const [name, spec] of Object.entries(pkg[section] ?? {})) {
      ...
      let specs = byName.get(name);
      if (!specs) byName.set(name, (specs = new Map()));
      let locations = specs.get(spec);
      if (!locations) specs.set(spec, (locations = []));
      locations.push(`${manifest} (${section})`);
    }
  }
}
